...
class MusicSpec extends AnyFreeSpec {
  ...
  "Silencio" in {
    val Success((silencioBlanca, _)) = tocable.parse("_")
    val Success((silencioNegra, _)) = tocable.parse("-")
    val Success((silencioCorchea, _)) = tocable.parse("~")
  ...
  "Sonido" in {
    val Success((unSonido, _)) = tocable.parse("5C#1/16")
    unSonido shouldBe Sonido(Tono(5, Cs), SemiCorchea)
  }

  "Acorde" - {
    "Acorde con calidad" in {
      val Success((unAcorde, _)) = tocable.parse("5CM1/4")
      unAcorde shouldBe C.acordeMayor(5, Negra)
    }
    "Acorde con notas" in {
      val Success((unAcorde, _)) = tocable.parse("6A+6C#+6G1/8")
      unAcorde shouldBe Acorde(List(Tono(6, A), Tono(6, Cs), Tono(6, G)), Corchea)
    }
  }
